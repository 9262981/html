...
//               reg = new RegExp("\\s|\\,|\\.|\\!|\\'|\\\"|\\;|\\:|\\t|\\n|\\r", "g");
    ...
    *Title: 京东618大家电推荐模板
    *Date: 20260421
    *Author: cdliqiaojun@jd.com
    *Description:
    *通过HTML模板获取skuid， 
    *然后根据地区请求真实skuid，   
    *最后将真实product渲染在页面。
    */
    realSkuInfo: function(args){
        var oldIds = [],
            _this = jQuery(this),
            oldSkuid = _this.find('input'),
            tpl = _this.find('ul').html();      
            
        for (var i = 0; i < oldSkuid.length; i++) {
            oldIds.push(jQuery(oldSkuid[i]).val())
        }

        jQuery.when(getRealGoods(oldIds.join(','), 'recomCallback')).done(function(data, newids){

            _this.find('ul').html(makeTpltoHtml(data,tpl));
    ...
    realCombInfo: function(){
        var oldIds = [],
            _this = jQuery(this),
            mainSkuid = _this.find('div[mainsku]').attr('mainsku'),
            subSkuid = _this.find('li[combsku]'),
            mainTpl = _this.find('div[mainsku]').html(),
            subTpl = jQuery(_this.find('li[combsku]')[0]).prop('outerHTML'),
            totalTpl = _this.find('.jTotal').html();

        jQuery.when(getRealGoods(mainSkuid, 'combMainCallback')).done(function(data, newids){
            _this.find('div[mainsku]').html(makeTpltoHtml(data, mainTpl));
            _this.find('.jTotal').html(makeTpltoHtml(data, totalTpl));
            skuIdPriceObj.localPriceRefresh(_this);
            _this.lazyLoad();
        })

        for (var i = 0; i < subSkuid.length; i++) {
            oldIds.push(jQuery(subSkuid[i]).attr('combsku'));
        }

        jQuery.when(getRealGoods(oldIds.join(','), 'combSubCallback')).done(function(data, newids){
            _this.find('li[combsku]').closest('ul').html(makeTpltoHtml(data, subTpl));            
            skuIdPriceObj.localPriceRefresh(_this);
            _this.lazyLoad();
        })
    },
    realClassRecom:function(){
         var oldIds = [],
            _this = jQuery(this),
            oldSkuid = _this.find('li[data-skuid]');

        for (var i = 0; i < oldSkuid.length; i++) {
            oldIds.push(jQuery(oldSkuid[i]).data('skuid'));
        }

        jQuery.when(getRealGoods(oldIds.join(','), 'classRecomCallback')).done(function(data, newids){            
            for (var i = 0; i < oldSkuid.length; i++) {
                var itemTpl = jQuery(oldSkuid[i]).html();
                for (var j = 0; j < data.length; j++) {
                    if(data[j].goodsId == newids[jQuery(oldSkuid[i]).data('skuid')].realSkuId){
                        jQuery(oldSkuid[i]).html(jshop.module.renderHTML(itemTpl, data[j]));
                    }
                }
            }            
            skuIdPriceObj.localPriceRefresh(_this);
...
//获取分地区后真实产品信息
function getRealGoods(skuIds, callback){
    var deff = jQuery.Deferred();
    jQuery.ajax({
        url : 'http://c0.3.cn/stock/arealimit',
        data :{
        skuIds: skuIds,
        area: getArea(),
        ch:1},
        ...
        success : function(newids){
            jQuery.ajax({
                url: 'http://act.jshop.jd.com/goodsInfo.html',
                data: {
                    skuIds: makeSkuids(newids),
                },
                dataType : 'jsonp',
                success: function(data){
                    deff.resolve(data, newids);
                }
            })
        }
    })
...
function getArea(){
    var cookies =  CookieUtil.getCookie('ipLoc-djd') ? CookieUtil.getCookie('ipLoc-djd') : '1-72-0-0';
    return cookies.split('-').join('_');
    //return '1_72_0_0'
}
//拼接skuid为字符串
function makeSkuids(data){
    var realSkuidA = [];
    jQuery.map(data, function(v, k){
        realSkuidA.push(v.realSkuId);
    })
    return realSkuidA.join(',');
}
//根据返回的数据填写模板
function makeTpltoHtml(data, tpl){
    htmlStr = '';
    for (var i = 0; i < data.length; i++) {
        htmlStr += jshop.module.renderHTML(tpl, data[i]);
    }
    return htmlStr;
}
